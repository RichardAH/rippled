...
//   ignore_default: bool           // do not return lines in default state (on this account's side)
// }
Json::Value
doAccountLines(RPC::JsonContext& context)
{
    ...
    // this flag allows the requester to ask incoming trustlines in default state be omitted
    bool ignoreDefault = params.isMember(jss::ignore_default);
    
    Json::Value& jsonLines(result[jss::lines] = Json::arrayValue);
    VisitData visitData = {{}, accountID, hasPeer, raPeerAccount, ignoreDefault, 0, nullptr};
    ...
    if (params.isMember(jss::marker))
    {
        ...
        {
            // even though we're starting our search here we don't include the first entry
            // in this edge case
